...
export const DemoDelete = () => {
  ...
  useEffect(() => {
    ...
    const b1 = new BulletLog({
      title: "delete by _id (from body)",
      description: `
      const response = await createDeclarativeBulletApi()
      .body({ _id: sampleData[0]._id, newName: "newName", myDate: new Date() })
      .collection((c) =>
        c.name(FIND_COLLECTION).method(BULLET_METHOD.DELETE_ONE)
      )
      .execute()
      `,
    });
    b1.func = () => deleteBy_id(b1);
    list.push(b1);
    /* #endregion */
    ...
    const b2 = new BulletLog({
      title: `delete by guid (from body) - increment age by 1
    ...
    b2.func = () => deleteBy_GuidWithBody(b2);
    ...
    const b3 = new BulletLog({
      ...
      const response = await createDeclarativeBulletApi()
      .collection((c) =>
        c.name(FIND_COLLECTION).method(BULLET_METHOD.DELETE)
      )
      .execute()
      `,
    });
    b3.func = () => deleteAll(b3);
  ...
  const deleteBy_id = async (bulletLog: BulletLog) => {
  ...
  /* #region  Find By _id from body */
  const deleteBy_GuidWithBody = async (bulletLog: BulletLog) => {
    await checkPrerequisites();
    const sampleData = getSampleData();
  ...
  /* #region  Find By _id from body */
  const deleteAll = async (bulletLog: BulletLog) => {
    await checkPrerequisites();
    const sampleData = getSampleData();
    ...
    const response = await createDeclarativeBulletApi()
      .collection((c) => c.name(FIND_COLLECTION).method(BULLET_METHOD.DELETE))
      .execute({
        beforeSendingRequest: (apiBulletJSON: any) => {
          apiBulletRequest = apiBulletJSON;
        },
  ...
  return (
    <>
      ...
      <div>NOTE: all the find criterias can be also applied to delete</div>
